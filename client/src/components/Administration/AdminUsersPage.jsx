        ...
        <Container className="mt-4">
            ...
            {loading ? (
                ...
                <Table striped bordered hover responsive className="shadow-sm mb-5">
                    ...
                    <tbody>
                    {users.length === 0 ? (
                        <tr><td colSpan="4" className="text-center">Brak użytkowników</td></tr>
                    ) : (
                        users.map((user) => (
                            <tr key={user.id || user.login}>
                                <td>{user.login} <br/><small className="text-muted">{user.firstName} {user.lastName}</small></td>
                                <td>{user.role?.name || user.role}</td>
                                <td>
                                    {user.active ? <Badge bg="success">Aktywne</Badge> : <Badge bg="secondary">Nieaktywne</Badge>}
                                </td>
                                <td>
                                    <Button
                                        size="sm"
