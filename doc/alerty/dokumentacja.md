...
## Podsumowanie
Moduł Alarmowania i Alertów stanowi punkt nadzoru bezpieczeństwa w systemie, integrując dane pochodzące z trzech niezależnych źródeł: bieżącej analizy sensorów, predykcji przyszłych awarii oraz zgłoszeń manualnych od personelu. Dzięki zastosowaniu architektury opartej na usługach działających w tle (`AlertAnalysisService`, `AlertPredictionService`). Moduł generuje ustandaryzowany obiekt Alert, który jest wysyłany do wszytkich subskrybentów modułu. Jednym z subskrybentów modułu jest dziennik zdarzeń, który pobiera obiekt alert i zapisuje go do bazy danych. Dziennik zdarzeń jest możliwy do wyświetlenia przez Administartorów i inżynierów stystemu. Użytkownicy natomiast mogą tworzyć obiekty alert o wadze SOS. System zapewnia ciągły, 24-godzinny monitoring bez konieczności stałej ingerencji operatora.
